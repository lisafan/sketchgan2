...
parser.add_argument("--input_dir", required=False, help="path to folder containing images")
parser.add_argument("--output_dir", required=False, help="output path")
parser.add_argument("--operation", required=False, default='combine', choices=["grayscale", "resize", "blank", "combine", "edges", "move"])
...
def combine(src, src_path):
    ...
    # src_path = '/a/h/jkrone02/..../sketch/tx000/airplane/n2131-1.png'
        
    classdir = os.path.split(os.path.split(src_path)[0])[1]
    filename = os.path.basename(src_path).split('-')[0]
    sibling_path = os.path.join('/a/h/jkrone02/Downloads/rendered_256x256/256x256/photo/tx_000100000000', classdir, filename + '.jpg')
...
def find(d):
    ...
    for dirname in os.listdir(d):
        for filename in os.listdir(d+'/'+dirname):
            _, ext = os.path.splitext(filename.lower())
	    if ext == ".jpg" or ext == ".png":
	        result.append(os.path.join(d, dirname, filename))
    result.sort()
    return result

def move():
    files = []
    path = '/a/h/lfan01/pix2pix-tensorflow/sketch_photo_pairs'
    ...
    total = len(files)
    print("Found ",total, " files. Sorting file names")
    files.sort()
    
    print("Moving val files")
    for i, filename in enumerate(files[0::6]):
	if i%1000==0:
	    print("  ",i,"/",total/6)
	os.rename(path+'/'+filename, path+'/val/'+filename)
    print("Moving test files")
    for i, filename in enumerate(files[1::6]):
	if i%1000==0:
	    print("  ",i,"/",total/6)
	os.rename(path+'/'+filename, path+'/test/'+filename)
...
def main():
    ...
    skipped = 0
    for src_path in find('/a/h/jkrone02/Downloads/rendered_256x256/256x256/sketch/tx_000100000000'):
        # '/a/h/jkrone/.../sketch/tx_000/airplane/n324021-1.png'
        name, _ = os.path.splitext(os.path.basename(src_path))

        dst_path = os.path.join('/a/h/lfan01/pix2pix-tensorflow/sketch_photo_pairs', name + ".png")
