...
def create_model(inputs, targets, classes_real, classes_fake):
    ...
    with tf.name_scope("discriminator_loss"):
        ...
        num_classes_const = tf.constant(NUM_CLASSES, shape=shape, dtype=tf.int64)
        A_const = tf.constant(2, shape=shape, dtype=tf.int64) 
        B_const = tf.constant(3, shape=shape, dtype=tf.int64) 
        C_const = tf.constant(4, shape=shape, dtype=tf.int64)

        # predictions
        real_softmax = tf.nn.softmax(real_outputs, dim=-1)	
        fake_softmax = tf.nn.softmax(fake_outputs, dim=-1)
        real_class_pred = tf.argmax(real_softmax, axis=1)
        fake_class_pred = tf.argmax(fake_softmax, axis=1)
        ...
        # penalty calc
        real_penalty = tf.select(real_binary, A_const, real_penalty)
        real_penalty = tf.select(real_class_binary, B_const, real_penalty)
        real_penalty = tf.select(real_, C_const, real_penalty)
        fake_penalty = tf.select(fake_binary, A_const, fake_penalty)
        fake_penalty = tf.select(fake_class_binary, B_const, fake_penalty)
        fake_penalty = tf.select(fake_, C_const, fake_penalty)
        # calculate loss
        ...
        discrim_real_supervised_loss = tf.reduce_sum(tf.prod(real_cross_entropy, real_penalty))
        discrim_fake_supervised_loss = tf.reduce_sum(tf.prod(fake_cross_entropy, fake_penalty))
...
def main():
    if tf.__version__ != "1.0.0" and tf.__version__ != "1.0.1":
        raise Exception("Tensorflow version 1.0.0 or greater required")
